pipeline {
    ...
    stage('Send Docker file to Ansible Server over SSH'){
        sshagen(['ansible_demo']){
            ...
            sh 'scp /var/lib/jenkins/workspace/pipeline-demo/* ubuntu@172.31.81.4:/home/ubuntu/' 
        }
    }
    ...
    stage('Push Docker Image to Docker Hub'){
        sshagent(['ansible_demo']){
            withCredentials([string(credentialsId: 'dockerhub_passwd', variable: 'dockerhub_passwd')]){
                sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.81.4 docker login -u uoekreh -p ${dockerhub_passwd}" 
     ...
     stage('Copy Files From Ansible to Kubernetes Server'){
         sshagent(['kubernetes_server']){
             ...
             sh 'scp /var/lib/jenkins/workspace/pipeline-demo/* ubuntu@172.31.19.112:/home/ubuntu/' 
         }
     }
